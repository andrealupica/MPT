...
if(($_SESSION['email']!="" OR $_SESSION['email']!=null) AND ($_SESSION["amministratore"]==1)){
	...
	while($row = $result->fetch_assoc()){
		...
		// nome della tabella
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell($width,$height,$corso." ".$inizio." - ".$fine,0,0,'L',0);
		// vado a capo
		$pdf->Ln($height);
		...
		// scrittura del nome delle colonne
		for ($i=1; $i <9 ; $i++) {
		...
		// query
		$sql = "SELECT cl.cla_nome AS 'classe',pia_gruppo AS 'gruppo',pia_sem AS 'semestre' from pianifica pia
		join corso co on co.cor_id = pia.cor_id
		...
		// selezioni il numero del semestre sul ciclo formativo ( da 1 a 8)
		while($row = $res->fetch_assoc()){
			...
			if(strpos($row['classe'],'B')!==false){
				...
				//echo $row["materia"]." sem: ".$nSem." ".$row["gruppo"]."<br>";
				// inserisco in un array bidimensionale nel primo il semestre e nel seconodo il gruppo
		...
		//echo "grandezza array: ".count($array)."<br>";
			...
			for ($i=0; $i < $riga ; $i++) { // righe
				for ($j=1; $j <=8 ; $j++) { // colonne
						$gruppo = $array[$j][$i];
						if($gruppo != null AND $gruppo!="" AND $array[$j][$i]!=$array[$j][$i-1]){ // se non è nullo e il gruppo non è quello precedente
							...
							$pdf->Cell($width,$height*4,$stringa,1,0,'L',0);
						}
					else{
